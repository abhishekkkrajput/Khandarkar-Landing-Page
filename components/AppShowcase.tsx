        ...
        <div className="max-w-7xl mx-auto px-6 overflow-hidden">
            <div className="flex flex-col lg:flex-row items-center gap-20">
                ...
                <div className="w-full lg:w-1/2 relative z-10 animate-fade-in-up">
                    <div className="inline-block px-4 py-1.5 bg-white/10 backdrop-blur-md rounded-full text-[#F5E0A3] text-xs font-bold mb-8 uppercase tracking-[0.2em] border border-[#D4AF37]/30 shadow-lg hover:bg-white/20 transition-colors cursor-default">
                        Pioneering Tradition
                    </div>

                    <h2 className="text-4xl md:text-6xl font-serif text-white mb-8 leading-[1.1] drop-shadow-xl">
                        Experience The <br />
                        <span className="text-gold-plated italic relative z-10">Future of Nikah</span>
                    </h2>
                    ...
                    <div className="grid grid-cols-2 gap-x-8 gap-y-10 mb-12">
                        {[
                            ...
                            <div key={i} className="flex flex-col group cursor-default animate-fade-in-up" style={{ animationDelay: `${i * 100}ms` }}>
                                <div className="flex items-center gap-3 mb-2">
                                    <span className="text-xl group-hover:scale-125 transition-transform duration-300 filter drop-shadow-md">{item.icon}</span>
                                    <span className="text-[#D4AF37] font-bold text-xs uppercase tracking-[0.2em]">{item.label}</span>
                                </div>
                                <span className="text-white text-lg font-serif tracking-wide border-l-2 border-[#D4AF37]/30 pl-3 group-hover:border-[#D4AF37] transition-all duration-500 user-select-none">{item.val}</span>
                            </div>
                        ))}
                    </div>

                    <button className="px-10 py-4 bg-white text-[#B6337D] font-bold rounded-full transition-all hover:scale-105 active:scale-95 shadow-[0_20px_40px_rgba(0,0,0,0.2)] hover:shadow-[0_25px_50px_rgba(255,255,255,0.2)] uppercase tracking-widest text-xs flex items-center gap-3 group relative overflow-hidden">
                        <span className="relative z-10">Download Now</span>
                        <svg className="w-4 h-4 transform group-hover:translate-x-1 transition-transform relative z-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                ...
                <div className="w-full lg:w-1/2 relative flex justify-center lg:justify-end">
                    ...
                    <div className="relative z-20 transform rotate-[-6deg] hover:rotate-0 transition-transform duration-1000 ease-out animate-float">
                        <div className="w-[340px] h-[680px] bg-[#1a1a1a] rounded-[3.5rem] border-[12px] border-[#2a2a2a] shadow-[0_50px_100px_rgba(0,0,0,0.8)] overflow-hidden relative ring-1 ring-white/10">
                            {/* Reflection */}
                        ...
                        <div className="absolute top-20 -left-12 bg-white/10 backdrop-blur-xl border border-white/20 p-4 rounded-2xl shadow-2xl animate-bounce" style={{ animationDuration: '3s' }}>
                            <div className="flex items-center gap-3">
                                <div className="w-10 h-10 rounded-full bg-[#B6337D] flex items-center justify-center text-white shadow-lg">❤️</div>
                                <div>
